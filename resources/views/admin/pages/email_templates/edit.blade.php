	...
	<header class="page-header">
        ...
        <div class="right-wrapper text-right">
          <ol class="breadcrumbs">
            ...
            <li><span>Edit Email Templates</span></li>
          </ol>
	...
	<div class="col">
		<section class="card">
			<header class="card-header">
				...
				<h2 class="card-title">Edit Template</h2>
			</header>
			<div class="card-body">
				<form class="form-horizontal form-bordered" action="{{ action('EmailTemplateController@update',['id'=>$template->id])}}" method="post" enctype="multipart/form-data">
					@csrf
					@method('PUT')
					<div class="form-group row">
						<label class="col-lg-2 control-label text-lg-right pt-2" for="inputHelpText">Name</label>
						<div class="col-lg-6">
							<input type="text" class="form-control{{ $errors->has('template_name') ? ' is-invalid' : '' }}" name="template_name" value="{{$template->template_name}}">
					...
					<div class="form-group row">
						...
						<div class="col-lg-10">
							<textarea name="template" data-plugin-markdown-editor rows="10">{{$template->template}}</textarea>
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-lg-6">
							<select  class="form-control populate" name="status">
									<option value="active" {{(($template->status == 'active' )?'selected=true':'')}}>active</option>
									<option value="inactive" {{(($template->status == 'inactive' )?'selected=true':'')}}>Inactive</option>
							</select>
						</div>
					</div>
