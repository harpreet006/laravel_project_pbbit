              ...
              <div class="card-body">
                <table class="table table-bordered table-striped mb-0" id="datatable-default">
                  ...
                  <tbody>
                      ...
                      <td>{{$topic->title}}</td>
                      <td>{{ substr($topic->description, 0, 150)}}</td>
                      <td>{{ @$topic->chapter->title}}</td>
                      <td>{{$topic->price}}</td>
                      <td>{{ count($topic->document).' / '.count($topic->video) }}</td>
                      <td><label class="label label-{{ ($topic->publish)?'success':'danger' }}"> {{ ($topic->publish)?'Publish':'Unpublish' }} </label></td>
                      <td>{{date('d-m-Y',strtotime($topic->created_at))}}</td>
                      <td> 
                          <a href="{{'topics/'.$topic->id}}">
                            <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                          </a>
                          <a href="" class="delete" onclick="" id="{{$key}}" msg="{{$topic->is_buyed()}}">
                            <i class="fa fa-remove" aria-hidden="true"></i>
                          </a>
                          <form action="{{action('TopicController@destroy',['id' => $topic->id])}}" method="POST" id="delete-form-{{$key}}">
                              @method('DELETE')
                               @csrf
                          </form>
                      </td>
